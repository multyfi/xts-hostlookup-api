...
module.exports = class XTSEmitter {
   ...
   *   localIP parameter is used to bind the socket connection to a specific local IP address.
   *
   */
  constructor(url, localIP) {
  ...
  /**
   * set the localIP value by providing the localIP in the input
   *
   * @param {string} localIP
   *  localIP parameter is used to bind the socket connection to a specific local IP address.
   */
  ...
  /**
   * Returns localIP used to bind the socket connection.
   *
   *
   * @return
   *   the localIP used to bind the socket connection to a specific local IP address.
   */
  get localIP() {
  ...
  init(reqObject) {
    ...
    // Check if URL contains a path component (for custom endpoints like /1hostlookup)
    var socketPath = '/socket.io';
    var connectionUrl = this.url;
    
    // Parse URL to check for custom path
    if (this.url.includes('hostlookup') || this.url.includes('/interactive') || this.url.includes('interactive')) {
      // Extract the base URL and custom path
      var urlParts = this.url.split('://');
      if (urlParts.length === 2) {
        var protocol = urlParts[0];
        var remainder = urlParts[1];
        var pathIndex = remainder.indexOf('/', remainder.indexOf(':') + 1);
        
        if (pathIndex > -1) {
          var basePart = remainder.substring(0, pathIndex);
          var pathPart = remainder.substring(pathIndex);
          connectionUrl = protocol + '://' + basePart;
          socketPath = pathPart + '/socket.io';
        }
      }
    }
    ...
    console.info('Socket Options: ', socketOptions);

    // Add custom agents for local IP binding if specified
    if (this.localIP) {
      const agentOptions = { localAddress: this.localIP };
      socketOptions.agent = this.url.startsWith('https') 
        ? new https.Agent(agentOptions)
        : new http.Agent(agentOptions);
      
      // For socket.io-client v2.x, also set transportOptions
      ...
      logger.logFile('Socket connection will use local IP: ' + this.localIP);
    }

    // path: "/interactive/socket.io",
    console.info('Connecting to socket with path:', socketPath);
    console.info('Connection URL: ' + connectionUrl);
    this.socketMD.socketMarketData = socketIoClient(connectionUrl, socketOptions);
